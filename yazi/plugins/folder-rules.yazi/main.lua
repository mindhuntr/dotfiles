local function setup()
	ps.sub("cd", function()
		...
		elseif cwd:ends_with("wallpapers") or cwd:ends_with("Telegram Desktop") then
			ya.emit("sort", { "btime", reverse = true, dir_first = true })
		elseif cwd:ends_with("Pictures") or cwd:ends_with("videos") then
			ya.emit("sort", { "btime", reverse = true, dir_first = true })
		elseif cwd:ends_with("pins") then
